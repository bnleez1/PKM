---
title: "(RO) Fixing run-on sentences: A comprehensive guide"
tags:
  - WritingErrorCode
  - GrammarSyntax
notes: []
gh-publish: true
gh-path: content/Assets/Pages
gh-published: true
gh-published-url: "https://bnleez1.github.io/PKM/assets/pages/(ro)-fixing-run-on-sentences-a-comprehensive-guide"
---

# (RO) Fixing run-on sentences: A comprehensive guide


💡 See also [[(CS) Comma Splice]].

# Run-on sentence

A run-on sentence occurs when two or more independent clauses (clauses that can stand alone as separate sentences) are improperly joined without appropriate punctuation or conjunctions. These sentences can be confusing and difficult to read due to their improper structure. There are several effective methods to correct run-on sentences:

### 1. Use a Period

- **Separate the Independent Clauses:** The simplest way to fix a run-on is to split the clauses into separate sentences using a period.

- **Example:**

    - Run-On: "She loves reading she goes to the library every week."

    - Corrected: "She loves reading. She goes to the library every week."

### 2. Use a Semicolon

- **Join Clauses with a Semicolon:** If the clauses are closely related, a semicolon can be used to join them more effectively than a period.

- **Example:**

    - Run-On: "He didn’t study he failed the test."

    - Corrected: "He didn’t study; he failed the test."

### 3. Use a Comma and a Coordinating Conjunction

- **Use a Comma Before a Conjunction:** If the clauses are closely connected, you can use a comma followed by a coordinating conjunction (for, and, nor, but, or, yet, so).

- **Example:**

    - Run-On: "It was raining we stayed indoors."

    - Corrected: "It was raining, so we stayed indoors."

### 4. Use a Subordinating Conjunction

- **Create a Complex Sentence:** You can turn one of the independent clauses into a dependent clause using a subordinating conjunction (because, since, although, if, when, etc.).

- **Example:**

    - Run-On: "She was tired she stayed awake."

    - Corrected: "Although she was tired, she stayed awake."

### 5. Use a Colon or Dash

- **Colon for Explanation or List:** If the second clause explains or expands on the first, a colon can be appropriate.

- **Dash for Emphasis or Interruption:** A dash can be used for emphasis or to indicate an abrupt change in thought.

- **Example:**

    - Run-On: "She has two hobbies reading and painting."

    - Corrected: "She has two hobbies: reading and painting."

### Examples of Correcting Run-On Sentences

- **Original Run-On:** "The weather was bad they canceled the trip."

- **Corrected Versions:**

    - Using a Period: "The weather was bad. They canceled the trip."

    - Using a Semicolon: "The weather was bad; they canceled the trip."

    - Using a Conjunction: "The weather was bad, so they canceled the trip."

    - Using a Subordinating Conjunction: "Because the weather was bad, they canceled the trip."

Understanding how to identify and correct run-on sentences is essential for clear and effective writing. Each method of correction can subtly change the tone or emphasis of the sentence, so the choice depends on the writer's intended meaning and style.

## Exercise to better understand runon sentences

### Detecting and Correcting Run-On Sentences

**Part A: Identifying Run-On Sentences**

Determine if the following sentences are correct (C) or run-ons (R). Write "C" or "R" next to each sentence.

1. I love going to the park it's relaxing and enjoyable.

2. She bought apples, oranges, and bananas from the market.

3. The movie was exciting I didn't want it to end.

4. My brother plays soccer, he practices every evening.

5. We visited the museum yesterday, it was really interesting.

6. He finished his homework, and then he watched television.

**Part B: Correcting Run-On Sentences**

Rewrite each run-on sentence from Part A correctly. Use punctuation, conjunctions, or separate sentences as necessary.

1. ___

2. ___

3. ___

4. ___

5. ___

6. ___

**Part C: Correct the Paragraph**

Read the paragraph below. Identify and correct the run-on sentences. Rewrite the paragraph properly.

> Last weekend, we went hiking the weather was perfect and sunny. We packed sandwiches, fruits, and water we left early in the morning. The hike was challenging, we were tired but happy. At the top of the mountain, the view was incredible we took lots of photos. Hiking is a great activity it keeps us healthy and connected to nature.

**Part D: Create Your Own**

Write two original run-on sentences and then correct them.

**Run-On Sentences:** 

1. ___

2. ___

**Corrected Sentences:** 

1. ___

2. ___

- **Answer Key**

    **Part A:**

    1. R

    2. C

    3. R

    4. R

    5. R

    6. C

    **Part B (possible corrections):**

    1. I love going to the park because it's relaxing and enjoyable.

    2. The movie was exciting, and I didn't want it to end.

    3. My brother plays soccer; he practices every evening.

    4. We visited the museum yesterday. It was really interesting.

    **Part C (example correction):** Last weekend, we went hiking. The weather was perfect and sunny. We packed sandwiches, fruits, and water, and we left early in the morning. The hike was challenging, so we were tired but happy. At the top of the mountain, the view was incredible, and we took lots of photos. Hiking is a great activity because it keeps us healthy and connected to nature.


